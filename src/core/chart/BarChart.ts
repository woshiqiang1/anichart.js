...
export class BarChart extends BaseChart {
  ...
  async setup(stage: Stage, ani: Ani) {
    ...
    this.labelPlaceholder = this.showLabel ? this.maxLabelWidth : 0;
  ...
  getComponent(sec: number) {
    ...
    const scaleX: ScaleLinear<number, number, never> =
      this.getScaleX(currentData);
    ...
    // 获取非 NaN 的条目数
    const barCount = currentData.filter(
      (d) => !Number.isNaN(d[this.idField])
    ).length;
  ...
  protected getScalesBySec(sec: number) {
    ...
    return {
      x: scaleLinear(
        [minValue, maxValue],
        [
          this.margin.left + this.labelPlaceholder,
          this.shape.width -
            this.margin.left -
            this.margin.right -
            this.valuePlaceholder -
            this.rankLabelPlaceholder,
        ]
      ),
      y: scaleLinear(
  ...
  private barIsUp(cFrame: number, hisIndex?: number[]) {
    if (
      hisIndex != null &&
      cFrame > 0 &&
      hisIndex[cFrame] < hisIndex[cFrame - 1]
    ) {
      return true;
    }
    return false;
  ...
  private getBarComponent(options: BarOptions) {
    ...
    const defaultBarInfoOptions = {
      ...
      position: {
        ...
        y: options.shape.height - this.dy,
      },
      fontSize: options.shape.height * this.barFontSizeScale,
      font,
      fillStyle: "#fff",
